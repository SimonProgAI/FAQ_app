...
function Home({ uname, unameSetter }) {
  ...
  useEffect(() => {
    const categoryMap = categoryAreaArray.map((cat) => {
      ...
      return (
        <div>
          <button className="catRow" onClick={handleClickCat} key={cat.id}>
            {cat.name}
          </button>
        </div>
      );
  ...
  useEffect(() => {
    ...
    fetch(url, parameter)
      ...
      .then((data) => {
        let qnArr = data.user;
        let processedQnArr = qnArr.map((element) => {
          ...
          function handleClickQn(e) {
            ...
            console.log(
              `Question: ${element.question}, Response: ${element.response}`
            );
          }
          return (
            <button
